...
import { loadMovieInfo } from '../../../features/details/pageDetailsSlice';
...
export const Movie: React.FC<IProps> = ({ sectionName }) => {
  ...
  const onWatchLaterClick = async (waitingContent: IAdd[]) => {
    ...
    const waitMovieAdded = findWaitSection?.items.find(
      (item: { id: number }) => item.id === kinopoiskId
    );

    if (waitMovieAdded?.id)
      return dispatch(
        setNotification({ type: 'warning', text: 'Вы уже добавляли этот фильм в список' })
      );
    ...
    await updateDoc(doc(db, 'users', userData.email), {
      ...
      .then(() =>
        dispatch(setNotification({ type: 'success', text: 'Фильм добавлен в список ожидаемых' }))
      )
      .catch(() =>
  ...
  const onFavoriteClick = async (favoriteContent: IAdd[]) => {
    ...
    const favMovieAdded = findFavSection?.items.find(
      (item: { id: number }) => item.id === kinopoiskId
    );
    ...
    await updateDoc(doc(db, 'users', userData.email), {
      ...
      .then(() =>
        dispatch(setNotification({ type: 'success', text: 'Фильм добавлен в список любимых' }))
      )
      .catch(() =>
        dispatch(setNotification({ type: 'reject', text: 'Произошла ошибка, попробуйте снова' }))
      );
              ...
              <div className={classes.movie__inner}>
                ...
                <div className={classes.movie__info}>
                  ...
                  <ul className={classes.movie__list}>
                    ...
                    {genres.length && (
                      <MovieListItem
                        ...
                        content={genres.map((g, i) =>
                          i === genres.length - 1 ? g.genre : `${g.genre}, `
                        )}
                      />
                    )}
                    ...
                    {ratingMpaa && (
                      <MovieListItem
                        description='Возраст MRAA'
                        content={ratingMpaa.toUpperCase()}
                      />
                    )}
                    {filmLength && (
                      <MovieListItem
                        description='Продолжительность'
                        content={`${filmLength} мин`}
                      />
                    )}
                  </ul>
                </div>
